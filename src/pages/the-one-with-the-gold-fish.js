...
export const query = graphql`
  query {
    allFile(
      filter: { relativeDirectory: { eq: "the-one-with-the-gold-fish" } }
    ) {
      edges {
        node {
          childImageSharp {
...
const TheOneWithTheGoldFishPage = ({ data }) => {
  const images = data.allFile.edges.map(({ node }) => node.childImageSharp);

  return (
    <Layout>
      <SEO title="The One With The Gold Fish" />
      <div className="animate__animated animate__fadeIn">
        <div className="static max-h-screen overflow-y-hidden">
          <StaticImage
            src="../images/the-one-with-the-gold-fish/painting-00003.JPG"
            alt="the-one-with-the-gold-fish-cover"
            placeholder="blurred"
...
export default TheOneWithTheGoldFishPage;
